...
export class RulesService {
    ...
    renderer.codespan = (text: string) => {
      if (text.includes(':')) {
        ...
        if (type === 'rule') {
          ...
          if (minor && chosenNode) {
            chosenString += `.${minor}`;
            chosenNode = chosenNode.children ? chosenNode.children[(+minor) - 1] : null;
          }

          if (child && chosenNode) {
            chosenString += `.${child}`;
            chosenNode = chosenNode.children ? chosenNode.children[(+child) - 1] : null;
          }

          if (desc && chosenNode) {
            chosenString += `.${toRoman(desc)}`;
            chosenNode = chosenNode.subchildren ? chosenNode.subchildren[(+desc) - 1] : null;
          }

          if (descDesc && chosenNode) {
            chosenString += `${String.fromCharCode((+descDesc) + 96)}`;
            chosenNode = chosenNode.subchildren ? chosenNode.subchildren[(+descDesc) - 1] : null;
          }
  ...
  private getFormattedRules() {
    ...
    baseRules.forEach((rule, majorRuleIndex) => {
      ...
      rule.posttext = format(rule.posttext);
      ...
      (rule.children || []).forEach((childRule, minorRuleIndex) => {
        ...
        childRule.posttext = format(childRule.posttext);
        ...
        (childRule.children || []).forEach((grandchildRule, revRuleIndex) => {
          ...
          grandchildRule.posttext = format(grandchildRule.posttext);
